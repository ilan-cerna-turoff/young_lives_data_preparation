# File: Ethiopia covid survey with younger children round 1
# Date: 28/03/2022
...
cvd_r1_et <- clean(survey = survey, round = round, country = country, vars = vars_i_want,
                   linkage_var = linkage_var) 

#------------------------------------------------------------------------------
#### 2. PERFORMING A MANUAL CHECK ####
#------------------------------------------------------------------------------
summary(cvd_r1_et$df) # a summary of the final clean data
cvd_r1_et$vars_needing_careful_manual_check
cvd_r1_et$vars_not_found
dd <- cvd_r1_et$data_dictionary # vars included above as vars_i_want
dd_discards <- cvd_r1_et$discarded_data_dictionary # vars not included

#------------------------------------------------------------------------------
#### 3. CONVERT TO TIBBLE FOR DATA CLEANING/MANIPULATION    ####
#------------------------------------------------------------------------------
df1 <- as_tibble(cvd_r1_et$df)
rm(cvd_r1_et) #clean environment
...
df1 <- df1 %>%
  ...
  replace_na(list(hhun193cov1 = "missing", hhun192cov1 = "missing", hep_group = "missing")) %>%
