# File: Peru covid survey with younger children round 1
# Date: 28/03/2022
...
vars_i_want <- c("havphncov1","hep_group","typesite_fc","hhun192cov1","hhun193cov1")

cvd_r1_pe <- clean(survey = survey, round = round, country = country, vars = vars_i_want,
                   linkage_var = linkage_var) 

#------------------------------------------------------------------------------
#### 2. PERFORMING A MANUAL CHECK ####
#------------------------------------------------------------------------------
summary(cvd_r1_pe$df) # a summary of the final clean data
cvd_r1_pe$vars_needing_careful_manual_check
cvd_r1_pe$vars_not_found
dd <- cvd_r1_pe$data_dictionary # vars included above as vars_i_want
dd_discards <- cvd_r1_pe$discarded_data_dictionary # vars not included

#------------------------------------------------------------------------------
#### 3. CONVERT TO TIBBLE FOR DATA CLEANING/MANIPULATION    ####
#------------------------------------------------------------------------------
df1 <- as_tibble(cvd_r1_pe$df)
rm(cvd_r1_pe) #clean environment
...
df1 <- df1 %>%
  ...
  replace_na(list(havphncov1 = "missing", hep_group = "missing", hhun193cov1 = "missing", hhun192cov1 = "missing", typesite_fc = "missing")) %>%
