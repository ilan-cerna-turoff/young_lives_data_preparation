# File: Vietnam covid survey with younger children round 1
# Date: 28/03/2022
...
vars_i_want <- c("havphncov1","hhun192cov1","hhun193cov1")

cvd_r1_vt <- clean(survey = survey, round = round, country = country, vars = vars_i_want,
                   linkage_var = linkage_var) 

#------------------------------------------------------------------------------
#### 2. PERFORMING A MANUAL CHECK ####
#------------------------------------------------------------------------------
summary(cvd_r1_vt$df) # a summary of the final clean data
cvd_r1_vt$vars_needing_careful_manual_check
cvd_r1_vt$vars_not_found
dd <- cvd_r1_vt$data_dictionary # vars included above as vars_i_want
dd_discards <- cvd_r1_vt$discarded_data_dictionary # vars not included

#------------------------------------------------------------------------------
#### 3. CONVERT TO TIBBLE FOR DATA CLEANING/MANIPULATION    ####
#------------------------------------------------------------------------------
df1 <- as_tibble(cvd_r1_vt$df)
rm(cvd_r1_vt) #clean environment
...
## Clean labels ##
df1 <- df1 %>%
  mutate(across(c("havphncov1","hhun192cov1","hhun193cov1"), as.character)) %>%    
  replace_na(list(havphncov1 = "missing", hhun193cov1 = "missing", hhun192cov1 = "missing")) %>%
  mutate(across(c("havphncov1","hhun192cov1","hhun193cov1"), as.factor))

#------------------------------------------------------------------------------
#### 5. RENAME FOR DIFFERENCES BETWEEN ROUNDS              ####
#------------------------------------------------------------------------------
df1 <- df1 %>%
  rename(havphncov = havphncov1,
