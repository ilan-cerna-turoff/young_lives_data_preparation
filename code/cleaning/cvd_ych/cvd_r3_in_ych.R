# File: India covid survey with younger children round 3
# Date: 18/04/2022
...
cvd_r3_in <- clean(survey = survey, round = round, country = country, vars = vars_i_want,
                   linkage_var = linkage_var) 

#------------------------------------------------------------------------------
#### 2. PERFORMING A MANUAL CHECK ####
#------------------------------------------------------------------------------
summary(cvd_r3_in$df) # a summary of the final clean data
cvd_r3_in$vars_needing_careful_manual_check
cvd_r3_in$vars_not_found
dd <- cvd_r3_in$data_dictionary # vars included above as vars_i_want
dd_discards <- cvd_r3_in$discarded_data_dictionary # vars not included

#------------------------------------------------------------------------------
#### 3. CONVERT TO TIBBLE FOR DATA CLEANING/MANIPULATION    ####
#------------------------------------------------------------------------------
df1 <- as_tibble(cvd_r3_in$df)
rm(cvd_r3_in) #clean environment
...
df1 <- df1 %>%
  ...
  replace_na(list(ened02cov3 = "missing", hep_group_tc = "missing")) %>%
