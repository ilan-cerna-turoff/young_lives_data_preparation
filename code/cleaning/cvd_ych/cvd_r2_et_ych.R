# File: Ethiopia covid survey with younger children round 2
# Date: 29/03/2022
...
cvd_r2_et <- clean(survey = survey, round = round, country = country, vars = vars_i_want,
                   linkage_var = linkage_var) 

#------------------------------------------------------------------------------
#### 2. PERFORMING A MANUAL CHECK ####
#------------------------------------------------------------------------------
summary(cvd_r2_et$df) # a summary of the final clean data
cvd_r2_et$vars_needing_careful_manual_check
cvd_r2_et$vars_not_found
dd <- cvd_r2_et$data_dictionary # vars included above as vars_i_want
dd_discards <- cvd_r2_et$discarded_data_dictionary # vars not included

#------------------------------------------------------------------------------
#### 3. CONVERT TO TIBBLE FOR DATA CLEANING/MANIPULATION    ####
#------------------------------------------------------------------------------
df1 <- as_tibble(cvd_r2_et$df)
rm(cvd_r2_et) #clean environment
...
df1 <- df1 %>%
  ...
  replace_na(list(ened02cov2 = "missing", cureducov2 = "missing", hep_group_sc = "missing")) %>%
