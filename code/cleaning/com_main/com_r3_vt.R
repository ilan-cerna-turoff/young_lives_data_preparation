# File: Vietnam community survey round 3
# Date: 23/03/2022
...
com_r3_vt <- clean(survey = survey, round = round, country = country, vars = vars_i_want,
                   linkage_vars = linkage_vars) 

#------------------------------------------------------------------------------
#### 2. PERFORMING A MANUAL CHECK ####
#------------------------------------------------------------------------------
summary(com_r3_vt$df) # a summary of the final clean data
com_r3_vt$vars_needing_careful_manual_check
com_r3_vt$vars_not_found
dd <- com_r3_vt$data_dictionary # vars included above as vars_i_want
dd_discards <- com_r3_vt$discarded_data_dictionary # vars not included

#------------------------------------------------------------------------------
#### 3. CONVERT TO TIBBLE FOR DATA CLEANING/MANIPULATION    ####
#------------------------------------------------------------------------------
df1 <- as_tibble(com_r3_vt$df)
rm(com_r3_vt) #clean environment
...
df1 <- df1 %>%
  ...
  replace_with_na(replace = list(anydis = c("crop failure due to pest/disease", # var = natural disaster E+
  ...
  replace_with_na(replace = list(disrel = c("99"))) %>%
  group_by(commid) %>%
  mutate(across(c("disrel"), ~replace(., . %in% c("other institutions","yes, the govt"), "yes")),
         disrel = ifelse(anydis == "none", NA, disrel), #corresponding NA if no disaster occurred
         disrel = ifelse(commid == "vn007", "yes", disrel), #manual re-coding of errors
         disrel = ifelse(commid == "vn001", "yes", disrel),
         disrel = ifelse(commid == "vn005", "yes", disrel),
         disrel = ifelse(commid == "vn006", "yes", disrel),
         disrel = ifelse(commid == "vn015", "yes", disrel),
         disrel = ifelse(commid == "vn007", "yes", disrel),
         disrel = ifelse(commid == "vn016", "yes", disrel),
         disrel = ifelse(commid == "vn018", "yes", disrel),
         disrel = ifelse(commid == "vn019", "yes", disrel),
         disrel = ifelse(commid == "vn020", "yes", disrel),
         disrel = ifelse(commid == "vn028", "yes", disrel),
         disrel = ifelse(commid == "vn034", "yes", disrel),
         disrel = ifelse(commid == "vn035", "yes", disrel),
         disrel = ifelse(commid == "vn036", "yes", disrel),
...
df1 <- df1 %>%
  ...
  replace_with_na(replace = list(robbery = c("99"))) %>%
  mutate(thfcrm = ifelse((robbery == "no" & cttlthft == "no"), "no", "yes"),
...
df1 <- df1 %>% 
  mutate(across(c("trans1","trans2","trans3"), as.character),
         across(c("trans1","trans2","trans3"), ~replace(., . %in% c("bus","mototaxi (any auto)"), "yes")),
         across(c("trans1","trans2","trans3"), ~replace(., . %in% c("by foot","motorcycle","bicycle",
                                                                    "animal (horse/donkey)","car","other"), "no"))) %>%
