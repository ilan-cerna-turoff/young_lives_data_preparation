...
####      N. NOTES      ####
#--------------------------#
# This script contains the function low_memory_clean_v2() for merging and cleaning young_lives_data_preparation raw data files when R's memory is superseded AFTER linkage variable errors.
# low_memory_clean_v2() does the following: 
...
#
# WARNINGS: package requirements: haven, here, tidyverse must all be installed for low_memory_clean_v2() to run.
#           additionally, low_memory_clean_v2() expects that data files (both raw & intermediate) are named according 
#           to the conventions laid out in the various markdown files.

#--------------------------#
####     F. FUNCTION    ####
#--------------------------#
low_memory_clean_v2 <- function(data, vars, linkage_vars, missing_keys = default_missing_keys, case_sensitive = FALSE, compression = "none") {
...
#--------------------------#
  ...
  # 3b. Remove any dataframes from the data list that came back as NULL (i.e. because they had no variables we were asking for):
...
####  4. MERGE ALL DATA ####
#--------------------------#
  # 4a. Provide information on variables not found:
  ...
  # 4b. Create a data dictionary for the variables we do have:
  data_dict <- map2_dfr(.x = data, .y = names(data), ~get_dict(cdf = .x, cdf_name = .y)) %>%
    mutate(var_needs_manual_check = ifelse((var_type == "factor" | str_detect(var_definition, "id") | str_detect(var_definition, "number") | str_detect(var_definition, "index") | str_detect(var_definition, "age") | str_detect(var_definition, "size")), F, T)) %>%
    as_tibble()
  
  # 4c. Create a data dictionary for the variables we DO NOT have:
  ...
  # 4d. Specifies different options for the structure of merged data:
  if (compression != "none") {
    if (compression == "chop") {
